...
void FPAD_Init( void )
{
	...
	memset((void*)0x93003010, 0, 0x190); //clears a lot of pad stuff
	...
	//WPAD_Init(); // moving this up here seems to fix double init
	PAD_Init();
	WUPC_Init();
...
void FPAD_Update( void )
{
	...
	//if(!(ncfg->Config & NIN_CFG_AUTO_BOOT)) //Doesn't work half the time
		//WPAD_ScanPads();
/*	for(i = 0; i < WPAD_MAX_WIIMOTES; ++i)
	{
